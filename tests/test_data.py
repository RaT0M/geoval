...
class TestData(unittest.TestCase):
    ...
    def test_get_climatology_stdev_InvalidGeometry(self):
        x = self.D.copy()
        x.data = np.random.random((2, 3, 4, 5))
        x.time_cycle = 1
        with self.assertRaises(ValueError):
    ...
    def test_get_climatology_stdev(self):
        x = self.D.copy()
    ...
    def test_get_climatology_stdev_InvalidTimecycle(self):
        d = self.D.copy()
        if hasattr(d, 'time_cycle'):
            del d.time_cycle
        with self.assertRaises(ValueError):
            d.get_climatology_stdev()
    ...
    def test_add(self):
        ...
        c = x.add(y)
        self.assertTrue(
            np.all(np.abs(1. - c.data[0, 0, 0] /
                          (x.data[0, 0, 0] * 2. + 3.)) < 1.E-6))
        self.assertTrue(
            np.all(np.abs(1. - c.data[100, 0, 0] /
                          (x.data[100, 0, 0] * 2. + 3.)) < 1.E-6))
    ...
    def test_get_percentile(self):
        for p in [0.05, 0.5, 0.95]:
            ...
            res = stats.mstats.scoreatpercentile(
                self.D.data[:, 0, 0], p * 100.)
    ...
    def test_correlate1(self):
        ...
        # 1) correlation with itself (returns data objects)
    ...
    def testTemporalTrendNoTimeNormalization(self):
        ...
        # no object is returned (default)
        R, S, I, P = self.D.temporal_trend(return_object=False)
        self.assertEqual(R[0, 0], r_value)
        self.assertEqual(S[0, 0], slope)

        # TODO further tests for slope and significance
        R, S, I, P = self.D.temporal_trend(return_object=True)
...
#         ~ D.time = np.asarray([t1,t2,t3]).flatten()
# use old python pylab time definition to be compliant
# with the test results here
#         ~ D._oldtime = True
#         ~ data = pl.rand(len(D.time), 1, 1)
#         ~ data[8:, 0, 0] = np.nan
...
#        stop
    ...
    def test_weighting_matrix(self):
        ...
        self.assertFalse(r.mask[0, 0, 0] is False)
    ...
    def test_adjust_time(self):
        D = self.D.copy()
        # D._oldtime = True #use old time convention to be compliant
        # with test routines here
    ...
    def test_days_per_month(self):
        ref = {1: [31], 2: [28, 29], 3: 31, 4: 30, 5: 31,
    ...
    def test_diff(self):
        ...
        # another test of the t-test, taken from
        ...
        A = self.D.copy()
        B = self.D.copy()
        X = np.zeros((len(x), 1, 1))
        Y = np.zeros((len(y), 1, 1))
        X[:, 0, 0] = x
        Y[:, 0, 0] = y
        A.data = np.ma.array(X, mask=X != X)
        B.data = np.ma.array(Y, mask=Y != Y)
    ...
    def test_save_InvalidOption(self):
        ...
        with self.assertRaises(ValueError):
            self.D.save(testfile, varname='testvar', format='nc',
                        delete=True, mean=True, timmean=True)
    ...
    def test_save_netCDF(self):
        ...
        self.D.save(tempfile.mktemp(suffix='.nc'), varname='testvar',
                    format='nc', delete=True, mean=True)
        ...
        # read data from default, this should then have the same
        # variable name as self.D
    ...
    def test_interp_time_InvalidMethod(self):
        tref = self.D.num2date(pl.datestr2num(
            '2001-05-05') + np.arange(200) * 0.5 + 0.25)
        with self.assertRaises(ValueError):
    ...
    def test_interp_time_InvalidGeometry(self):
        d = self.D.copy()
        d.data = np.random.random((10, 20, 30, 40))
        tref = d.num2date(pl.datestr2num('2001-05-05') +
                          np.arange(200) * 0.5 + 0.25)
        with self.assertRaises(ValueError):
            d.interp_time(tref)
    ...
    def test_interp_time(self):
        ...
        tref = D.num2date(pl.datestr2num('2001-07-05') +
                          np.arange(20) * 0.5 + 0.25)
        # 5.July ... 14.July
        # ~ print tref
        ...
        if False:
            ...
            pl.plot(tref, yy, color='green',
                    label='reference interp', linestyle='--')
            pl.legend()
            pl.show()

        d = yy - I.data[:, 0, 0]
        # boundary effects at end of period, therefore last value not used
        self.assertFalse(np.any(np.abs(d[0:-1]) > 1.E-10))
    ...
    def test_save_ascii(self):
        ...
        self.D._save_ascii(self._tmpdir + os.sep +
                           'testexport.txt', delete=True)
        self.assertTrue(os.path.exists(
            self._tmpdir + os.sep + 'testexport.txt'))
    ...
    def test_apply_temporal_mask_WithInvalidMaskTimes(self):
        data_object = self.D
        with self.assertRaises(ValueError):
            data_object._apply_temporal_mask(
                np.random.random(data_object.nt + 1))
    ...
    def test_partial_correlation(self):
        ...
        y = x.copy()
        y.data = y.data + pl.rand(nt, ny, nx) * 1000.
        z = x.copy()
        z.data = z.data * pl.rand(nt, ny, nx) * 100.
        ...
        slope, intercept, rxy, p_value, std_err = stats.linregress(
            x.data[:, 0, 0], y.data[:, 0, 0])
        slope, intercept, rxz, p_value, std_err = stats.linregress(
            x.data[:, 0, 0], z.data[:, 0, 0])
        slope, intercept, rzy, p_value, std_err = stats.linregress(
            z.data[:, 0, 0], y.data[:, 0, 0])

        ref = (rxy - rxz * rzy) / \
            (np.sqrt(1. - rxz * rxz) * np.sqrt(1. - rzy * rzy))
    ...
    def generate_tuple(self, n=None, mask=True):
        ...
        y.data = y.data * z
        if mask:
            # mask some data so we have data with different masks
            y.data = np.ma.array(y.data, mask=z > 0.5)
        else:
            # mask some data so we have data with different masks
    ...
    def test_corr_single(self):
        ...
        # --- pearson
        slope, intercept, r, prob, sterrest = stats.mstats.linregress(
            y, x.data[:, 0, 0])
    ...
    def test_correlate(self):
        for n in [None, 100, 10, 5]:  # different size
            ...
            x1 = x.data[:, 0, 0]
            y1 = y.data[:, 0, 0]
            msk = (x1.mask is False) & (y1.mask is False)
            x2 = x1[msk]
            ...
            # PEARSON CORRELATION
            ##################################################################
            slope, intercept, r_value1, p_value1, std_err = (
                stats.mstats.linregress(x1, y1))  # masked
            slope, intercept, r_value2, p_value2, std_err = (
                stats.linregress(x2, y2))  # not masked
            r, p = x.correlate(y)
            ...
            # results from stats.linregress are used, as mstats is BUGGY!!
            ...
            # 1) test if scipy functions return similar results for
            #    masked/not masked arrays
    ...
    def test_bounding_box(self):
        D = self.D.copy()
        D.data = np.ma.array(
            pl.rand(10, 5, 8), mask=np.zeros((10, 5, 8)).astype('bool'))
    ...
    def test_fldmean(self):
        ...
        # testcase where some of data is not valid and different
        # weighting approaches are applied
        D = self.D.copy()
        ...
        # ... now check what happens if normalization factor is for ALL pixels
        # and not only the valid ones! --> should give 0.75
        D.weighting_type = 'all'
        r = D.fldmean()[0]
    ...
    def test_fldstd(self):
        ...
        # define testcase described under
    ...
    def test_areasum(self):
        ...
        # testcase where some of data is not valid and different weighting
    ...
    def test_set_timecycle(self):
        ...
        tref = rrule(MONTHLY, dtstart=start_time).between(
            start_time, stop_time, inc=True)  # monthly timeseries
        ...
        D.time[2] = pl.datestr2num(
            '2010-' + str(t.month).zfill(2) + '-' + str(t.day).zfill(2))
        D._set_timecycle()
        self.assertFalse(D._is_monthly())
        self.assertEquals(D.time_cycle, None)
    ...
    def test_get_valid_mask(self):
        ...
        D.data = np.ma.array(x, mask=x == 0)
        m = D.get_valid_mask()
        self.assertTrue(m[0, 0] is True)
        self.assertTrue(m[0, 1] is True)
        ...
        D.data = np.ma.array(x, mask=x == 0)
        m = D.get_valid_mask()
        self.assertTrue(m[0, 0] is False)
        self.assertTrue(m[0, 1] is True)
    ...
    def test_temporal_smooth(self):
        ...
        self.assertAlmostEqual(
            tmp[10:13, 0, 0].sum() / 3., y3a.data[11, 0, 0], 8)
        self.assertAlmostEqual(
            tmp[10:13, 1, 1].sum() / 3., y3a.data[11, 1, 1], 8)
        self.assertAlmostEqual(
            tmp[10:13, 1, 0].sum() / 3., y3a.data[11, 1, 0], 8)
...
#         ~ how to treat leap years ???
    ...
    def test_convert_timeYYYY(self):
        ...
        # no leap year
        d.time = np.asarray([1999.,
                             1999.0 + 23. / (365. * 24.),
                             1999.0 + 2. / 365. + 20. / (365. * 24.),
                             1999.0 + 59. / 365.,
                             1999. + 1. / (365. * 24.)])  # 23:00
        d._convert_timeYYYY()
        k = 0
        ...
        # no leap year
        d.time = np.asarray([1900., 1900.0 + 23. / (365. * 24.),
                             1900.0 + 2. / 365. + 20. / (365. * 24.)])  # 23:00
        d._convert_timeYYYY()
        k = 0
        ...
        # special leap year
        d.time = np.asarray([2000.,
                             2000.0 + 23. / (366. * 24.),
                             2000.0 + 2. / 366. + 20. / (366. * 24.),  # 23:00
                             2000.0 + 61. / 366. + 19. / (366. * 24.)])
        d._convert_timeYYYY()
        k = 0
...
# TODO would need to implement a test which ensures that the area weights are
# properly calculated, independent whether the input file format is supported
# by the CDO's or not.
    ...
    def test_mask_region(self):
        ...
        y = x.copy()
        y.mask_region(reg, return_object=False, method='full',
                      maskfile=None, force=False)
        y1 = y.copy()
        res2 = y.mask_region(reg, return_object=True,
                             method='full', maskfile=None, force=False)

        with self.assertRaises(ValueError):
            res3 = y.mask_region(
                reg, return_object=False, method='full',
                maskfile='no_valid_filename', force=False)

        y = x.copy()
        res4 = y.mask_region(reg, return_object=True,
                             method='full', maskfile=mfile, force=False)
        self.assertTrue(os.path.exists(mfile))

        y = x.copy()
        res5 = y.mask_region(reg, return_object=True,
                             method='full', maskfile=mfile, force=False)
        self.assertTrue(os.path.exists(mfile))

        y = x.copy()
        res6 = y.mask_region(reg, return_object=True,
                             method='full', maskfile=mfile, force=True)
        self.assertTrue(os.path.exists(mfile))
